...
from
   (select distinct on (se.subscription_id)
      ...
      rt.* 
   from
   ((select distinct on (cct.retry_id)
    pt.amount                refund_amount,
    st.transaction_date      refund_date,
    cct.transaction_number   tx_ref_num,
    st.id                    original_txn_id
  from gcsi.gaiam_division gd
    join gcsi.payment_transaction pt on pt.division_id = gd.uuid  --refund
    join gcsi.credit_card_refund_transaction ccrt on pt.id = ccrt.id
    join gcsi.payment_transaction pt2 on ccrt.refund_target_id = pt2.id --original
    join gcsi.credit_card_transaction cct on pt2.id = cct.id
    join gcsi.subscription_transaction st on st.invoice_id = pt2.invoice_id
    left join gcsi.auth_transaction auth on st.id = auth.id
    left join gcsi.shippable_subscription_transaction sst on st.id = sst.id
  where
    (auth is null or sst is not null) and --eliminate any auths
    st.txn_state = 'settled' and
    gd.name = 'gaiam tv' and
    pt.amount = pt2.amount -- full refund
  order by cct.retry_id, st.id)
 union
 -- PayPal  Refund Transactions
 (select
    pt.amount                           refund_amount,
    st.transaction_date                 refund_date,
    pprd.transaction_id :: varchar(40)  tx_ref_num,
    pprt.id                             original_txn_id
  from gcsi.gaiam_division gd
    join gcsi.payment_transaction pt on pt.division_id = gd.uuid  --refund
    join gcsi.pay_pal_refund_transaction pprt on pt.id = pprt.id
    join gcsi.payment_transaction pt2 on pprt.refund_target_id = pt2.id --original
    join gcsi.pay_pal_transaction ppt on ppt.id = pt2.id
    join gcsi.pay_pal_response_data pprd on ppt.last_response_id = pprd.id
    JOIN gcsi.subscription_transaction st on st.invoice_id = pt2.invoice_id
    LEFT join gcsi.auth_transaction auth on st.id = auth.id
    LEFT join gcsi.shippable_subscription_transaction sst on st.id = sst.id
  where st.txn_state = 'settled'
        and (auth is null or sst is not null) --eliminate any auths
        and gd.name = 'Gaiam TV'
        and pt.amount = pt2.amount)) rt
    join gcsi.subscription_transaction st on st.id = rt.original_txn_id
    join gcsi.subscription_plan_event spe on spe.id = st.subscription_plan_event_id
    join gcsi.subscription_event se on se.id = st.subscription_plan_event_id
...
from gcsi.subscription gs
 ...
 JOIN gcsi.subscription_transaction st ON st.subscription_plan_event_id = se.id and st.txn_state = 'SETTLED'
 LEFT JOIN gcsi.payment_transaction pt on st.invoice_id = pt.invoice_id and pt.action = 'Capture' and pt.amount > 0;

--alter table moiram.paid_subscriptions_tmp owner to dw_admin;
...
--alter table moiram.paid_subscriptions owner to dw_admin;
...
select ns.*,
       ...
       case when status in ('Active','Hold', 'AdminHold') then '2999-12-31'::date
            -- cancel date that is exactly the same as next_start is a rollover, not a cancellation
            when cancel_date = next_start then cancel_date
...
create index idxm_ns_sid on moiram.next_starts using btree (subscription_id);
 ...
 update moiram.subscriptions_tmp st set valid_to =
    case when status in ('Active','Hold', 'AdminHold') then '2999-12-31'::date
         ...
         -- cancel date that is exactly the same as next_start it's a rollover, not a cancellation
         when cancel_date = next_start then cancel_date
         when (start_date,cancel_date) overlaps (next_start, coalesce(next_end,current_date))
             then next_start - interval '1 day'
         when cancel_date is null and next_start is not null
...
where EXISTS
   (select subscription_id
    ...
    where st1.gcsi_user_id = st2.gcsi_user_id
      ...
      --strict overlapping, not just butting up against
      and  st1.valid_to > st2.start_date
      and  st1.subscription_age < st2.subscription_age
    )
...
-- if the hold starts and ends on the same day, it's a no-op
delete from moiram.tmp_holds 
where hold_start_date::date = hold_end_date::date;
...
-- dont add an 'End' activity if it's redundant
-- (same as cancel date or the next start is the same day)
insert into moiram.tmp_activity
(select distinct on (subscription_id) gcsi_user_id,
    subscription_id,
...
where end_date is not null 
  and (end_date::date > cancel_date::date or cancel_date is null)
  and (end_date::date < next_start_date::date or next_start_date is null));
...
-- dont add an 'Cancel' activity if it's redundant
-- (next start is the same day)
insert into moiram.tmp_activity (
select distinct
...
where cancel_date is not null
  and (cancel_date::date < next_start_date::date or next_start_date is null));
...
create view moiram.current_users

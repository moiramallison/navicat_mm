drop table if exists st_sub_master_30_60_90;


create table st_sub_master_30_60_90 as
select t3.*,
   case when paid_through_age >= 28 then 1 else 0 end sub_30_day,
   case when paid_through_age >= 58 and
             anniversary_60_date < '2015-01-15'::date and
             churn_30_day = 0 then 1
        when churn_60_day =1 and churn_30_day = 0 then 1  
        else 0
   end sub_60_day,
   case when paid_through_age >= 88 and
             anniversary_90_date < '2015-01-15'::date and
             churn_60_day = 0 then 1
        when churn_90_day =1 and churn_60_day =0 and churn_30_day = 0 then 1  
        else 0
   end sub_90_day
from
   (select t2.*,
       case when cancel_age <= 34 then 1 
            when paid_through_age <= 34 and cancel_age is not null then 1
            when cancel_age between 118 and 122 AND
                 long_hold = 1 then 1 
            else 0
       end   churn_30_day,
       case when cancel_age >  34 and cancel_age <= 64 then 1 
            when paid_through_age > 34 and paid_through_age  <= 64 and cancel_age > 64 then 1
            when cancel_age between 148 and 152 AND
                 long_hold = 1 then 1 
            else 0
       end   churn_60_day,
       case when cancel_age >  64 and cancel_age <= 94 then 1 
            when paid_through_age > 64 and paid_through_age  <= 91 and cancel_age > 94 then 1
    ...
    from
        ...
        (select gcsi_user_id, drupal_user_id, 
           ...
           subscription_start_date::date + 120 anniversary_120_date,
        ...
        where plan_id = 21891276
          ...
          and subscription_start_date <= '2015-12-15'
           and (onboarding_parent = 'Seeking Truth' or 
                user_behavior_segment = 'Seeking Truth')) t1
        where paid_through_age > 0) t2 )t3
;



drop table if exists st_video_views_master;
drop table if exists st_video_views_30_60_90;

create table st_video_views_30_60_90 as 
select
    drupal_user_id,
    period, 
...
from
    ...
    from
        ...
        from st_sub_master_30_60_90 sm3
...
create table st_video_views_master as
select 
    v1.drupal_user_id,
...
from st_sub_master_30_60_90 sm 
left join 
     (select * from st_video_views_30_60_90 where period = 1) v1
     on sm.drupal_user_id = v1.drupal_user_id
left join 
     (select * from st_video_views_30_60_90 where period = 2) v2
     on sm.drupal_user_id = v2.drupal_user_id
left join 
     (select * from st_video_views_30_60_90 where period = 3) v3
     on sm.drupal_user_id = v3.drupal_user_id;

drop table if exists st_guide_day_comp_30_60_90;

create table st_guide_day_comp_30_60_90 as 
select 
    drupal_user_id,
    period, 
...
from (select gd2.*,
    ...
    from
        ...
        join st_sub_master_30_60_90 sm3
          on gd1.drupal_user_id = sm3.drupal_user_id) gd2) gd3
group by drupal_user_id, period;



drop table if exists st_guide_optins_30_60_90;

create table st_guide_optins_30_60_90 as
select 
    drupal_user_id,
    period, 
...
drop table if exists st_guide_activity_master;

create table st_guide_activity_master as 
select gd4.*,
...
from
    ...
    from st_sub_master_30_60_90 sm 
   left join 
          (select * from st_guide_optins_30_60_90 where period = 1) g1
          on sm.drupal_user_id = g1.drupal_user_id
   left join 
          (select * from st_guide_optins_30_60_90 where period = 2) g2
          on sm.drupal_user_id = g2.drupal_user_id
   left join 
          (select * from st_guide_optins_30_60_90 where period = 3) g3
          on sm.drupal_user_id = g3.drupal_user_id) gd4   
left join 
      (select * from st_guide_day_comp_30_60_90 where period = 1) gd1
      on gd4.drupal_user_id = gd1.drupal_user_id
 left join 
      (select * from st_guide_day_comp_30_60_90 where period = 2) gd2
      on gd4.drupal_user_id = gd2.drupal_user_id
 left join 
      (select * from st_guide_day_comp_30_60_90 where period = 3) gd3
      on gd4.drupal_user_id = gd3.drupal_user_id;
     



drop table if exists st_sfss_30_60_90 cascade;

create table st_sfss_30_60_90 as 
select
    sm.gcsi_user_id ,
    sm.drupal_user_id ,
    sm.subscription_id, 
...
from st_sub_master_30_60_90 sm 
left join st_video_views_master vv
    on sm.drupal_user_id = vv.drupal_user_id
left join st_guide_activity_master gm
...
create or replace view st_sfss as select * from st_sfss_30_60_90;

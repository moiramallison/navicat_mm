...
drop table if exists ap_video_views_master;

create table ap_video_views_master as
select t1.*,
...
from 
	(select drupal_user_id,  
	min(days_since_last_video_view) days_since_last_video_view
	from ap_video_views_smry
	group by drupal_user_id) t1
left join 
     (select * from ap_video_views_smry where period = 1) v1
     on t1.drupal_user_id = v1.drupal_user_id
left join 
     (select * from ap_video_views_smry where period = 2) v2
     on t1.drupal_user_id = v2.drupal_user_id
left join 
     (select * from ap_video_views_smry where period = 3) v3
     on t1.drupal_user_id = v3.drupal_user_id;
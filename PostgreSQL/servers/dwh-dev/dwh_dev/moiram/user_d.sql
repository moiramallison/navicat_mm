...
drop table if exists tmp.refund_transactions;

create table tmp.refund_transactions as
select ref.*,
...
from
   ...
   ((select distinct on (cct1.retry_id)
    ...
    where seg.duration_period::interval > 'P1M'::interval
...
alter table tmp.refund_transactions owner to dw_admin;

create index idx_rt_sid on tmp.refund_transactions using btree (subscription_id);

drop table if exists tmp.paid_subscriptions_tmp;

create table tmp.paid_subscriptions_tmp as
...
select distinct subscription_id from tmp.paid_subscriptions_tmp;


alter table tmp.paid_subscriptions owner to dw_admin;

create index idx_ps_sid on tmp.paid_subscriptions using btree (subscription_id);

drop table if exists tmp.dwh_subscriptions;
create table tmp.dwh_subscriptions as 
select t.*
from
    (select
        ...
        case when seg.segment_type = 'TRIAL' and s.start_date > '20151120'
             then 1 else 0
    ...
    left join tmp.refund_transactions rt on rt.subscription_id = s.subscription_id
    left join tmp.paid_subscriptions psub on psub.subscription_id = s.subscription_id
    left join 
...
alter table tmp.dwh_subscriptions owner to dw_admin;

drop table if exists tmp.next_starts;
...
create table tmp.next_starts as
...
from  tmp.next_starts ns;
alter table tmp.subscriptions_tmp owner to dw_admin;
...
from
    (select   gcsi_user_id, subscription_id, 
        ...
        cancel_date,
        start_date
    from tmp.subscriptions_tmp) t
window w as (
    partition by gcsi_user_id
    order by paid_through_date, start_date
    rows between unbounded preceding and unbounded following);
 
create index idx_ns_sid on tmp.next_starts using btree (subscription_id);


 update tmp.subscriptions_tmp st set next_start = 
    (select next_start from tmp.next_starts ns where st.subscription_id = ns.subscription_id);
    
 update tmp.subscriptions_tmp st set next_end = 
    (select next_end from tmp.next_starts ns where st.subscription_id = ns.subscription_id);
    
 update tmp.subscriptions_tmp st set valid_to =
    case when status in ('Active','Hold', 'AdminHold') then '2999-12-31'::date
...
alter table tmp.t_uniq_subs owner to dw_admin;
create index idx_uniq_sid on tmp.t_uniq_subs using btree (subscription_id);
create index idx_uniq_gcsiid on tmp.t_uniq_subs using btree (gcsi_user_id);
...
alter table tmp.users_tmp owner to dw_admin;
...
alter table tmp.tmp_holds_prelim owner to dw_admin;
...
alter table tmp.tmp_holds owner to dw_admin;
...
alter table tmp.hold_overlaps2 owner to dw_admin;
...
drop table if exists tmp.user_activity;

create table tmp.user_activity as (
    select
        t1.gcsi_user_id,
        t1.subscription_id,
        t1.activity,
...
create index idx_ua_sid on tmp.user_activity using btree(subscription_id);
create index idx_ua_sidvf on tmp.user_activity using btree(subscription_id, valid_from);

alter table tmp.user_activity owner to dw_admin;
...
from tmp.user_activity u1
join tmp.user_activity u2
  on  u1.subscription_id = u2.subscription_id and
      u1.valid_from = u2.valid_from
where u1.activity = 'Start'
  and u2.activity = 'Hold Start';

alter table tmp.start_hold_tmp owner to dw_admin;

update tmp.user_activity ua
...
delete from tmp.user_activity ua using tmp.start_hold_tmp st
where ua.subscription_id = st.subscription_id
  and ua.valid_from = st.valid_from
...
drop table if exists tmp.segments;
...
alter table tmp.segments owner to dw_admin;

drop table if exists tmp.user_status;

create table tmp.user_status as
select useg.*,
...
from tmp.segments useg;

create index idx_ustat_sid on tmp.user_status (subscription_id);
    
alter table tmp.user_status owner to dw_admin;

-- these are subscription-level attributes not in t_subscriptions
drop table if exists tmp.subscription_attributes;

create table tmp.subscription_attributes as
...
alter table tmp.subscription_attributes owner to dw_admin;
create index idx_sattr_sid on tmp.subscription_attributes (subscription_id);

 -- REAL user behavior segment
drop table if exists tmp.user_behavior;

create table tmp.user_behavior as
...
alter table tmp.user_behavior owner to dw_admin;
create index idx_tub_uid on tmp.user_behavior (drupal_user_id);

-- onboarding_segment
drop table if exists tmp.onboarding_segments;

create table tmp.onboarding_segments as
(select t.drupal_user_id,
...
alter table tmp.onboarding_segments owner to dw_admin;
create index idx_tos_uid on tmp.onboarding_segments (drupal_user_id);

-- playlist activity segment
drop table if exists tmp.playlist_segments;

create table tmp.playlist_segments as
...
alter table tmp.playlist_segments owner to dw_admin;
create index idx_tps_uid on tmp.playlist_segments (drupal_user_id);

 -- guide segment

drop table if exists tmp.guide_segments;

create table tmp.guide_segments as
 (select distinct on (uid)
        uid drupal_user_id,
...
alter table tmp.guide_segments owner to dw_admin;
create index idx_tgs_uid on tmp.guide_segments (drupal_user_id);

-- cid code segment
drop table if exists tmp.campaign_segments;

create table tmp.campaign_segments as
    (select distinct on (gcsi_user_id)
        gcsi_user_id,
        campaign_segment segment_name
     from tmp.subscription_attributes
     where campaign_segment in ('My Yoga', 'Seeking Truth', 'Spiritual Growth')
     order by gcsi_user_id, subscription_id desc);

alter table tmp.campaign_segments owner to dw_admin;
create index idx_tcs_gcsiid on tmp.campaign_segments (gcsi_user_id);




drop table if exists tmp.user_attributes;

create table tmp.user_attributes as (
select t.*,
...
left join tmp.onboarding_segments os
   on t.drupal_user_id = os.drupal_user_id
left join tmp.user_behavior ubs
  on t.drupal_user_id = ubs.drupal_user_id
left join tmp.playlist_segments ps
  on t.drupal_user_id = ps.drupal_user_id
left join tmp.guide_segments gs
  on t.drupal_user_id = gs.drupal_user_id
left join tmp.campaign_segments cs
  on t.gcsi_user_id = cs.gcsi_user_id);

alter table tmp.user_attributes owner to dw_admin;
create index idx_uattr_user on tmp.user_attributes (gcsi_user_id);
create index idx_uattr_duid on tmp.user_attributes (drupal_user_id);

drop table if exists tmp.suh_segments;

create table tmp.suh_segments as
select distinct on (drupal_user_id)
    drupal_user_id,
    reporting_segment segment_name
from
       ...
       where coalesce(uid, client_uid) in
          (select drupal_user_id from tmp.user_attributes
...
update tmp.user_attributes ua
set user_behavior_segment =
(select segment_name from tmp.suh_segments ss

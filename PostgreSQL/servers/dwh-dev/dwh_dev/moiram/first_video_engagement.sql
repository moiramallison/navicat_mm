...
from
    ...
    group by user_id
    union
    select user_id drupal_user_id,70, min(id) suh_id
    from common.video_tmp vt 
    join common.video_d v on vt.nid = v.media_nid
    where watched >= (duration *.70)
    group by user_id
    union
    select user_id drupal_user_id,80, min(id) suh_id
    from common.video_tmp vt 
    join common.video_d v on vt.nid = v.media_nid
    where watched >= (duration *.80)
    group by user_id
    union
    select user_id drupal_user_id,90, min(id) suh_id
    from common.video_tmp vt 
    join common.video_d v on vt.nid = v.media_nid
    where watched >= (duration *.90)
    group by user_id
    union
    select user_id drupal_user_id,100, min(id) suh_id
    from common.video_tmp vt 
    join common.video_d v on vt.nid = v.media_nid
    where watched >= duration 
    group by user_id) foo
 join smfplayer_user_history suh on suh_id = suh.id;

drop table first_video_engagement;
...
select fu.drupal_user_id,
   case when (f0.suh_id = f1.suh_id  or f1.suh_id is null) and
             ...
             (f0.suh_id = f10.suh_id or f10.suh_id is null) and
             (f0.suh_id = f100.suh_id or f100.suh_id is null)
        then 0
        when (f1.suh_id = f3.suh_id  or f3.suh_id is null) and 
             ...
             (f1.suh_id = f10.suh_id or f10.suh_id is null)  and
             (f1.suh_id = f100.suh_id or f100.suh_id is null)
        then 1
        when (f3.suh_id = f5.suh_id  or f5.suh_id is null) and
             (f3.suh_id = f8.suh_id  or f8.suh_id is null) and
             (f3.suh_id = f10.suh_id or f10.suh_id is null)  and
             (f3.suh_id = f100.suh_id or f100.suh_id is null)
        then 3
        when (f5.suh_id = f8.suh_id  or f8.suh_id is null) and
             (f5.suh_id = f10.suh_id or f10.suh_id is null)  and
             (f5.suh_id = f100.suh_id or f100.suh_id is null)
        then 5
        when (f8.suh_id = f10.suh_id or f10.suh_id is null)  and
             (f8.suh_id = f100.suh_id or f100.suh_id is null)
        then 8
        when (f10.suh_id = f100.suh_id or f100.suh_id is null) 
        then 10
        when f100.suh_id is not null then 100
        end fve_min_bin,
   case when f100.suh_id is not null then 100
...
left join (select * from fve where minutes =8) f8 on fu.drupal_user_id = f8.drupal_user_id
left join (select * from fve where minutes =10) f10 on fu.drupal_user_id = f10.drupal_user_id
left join (select * from fve where minutes =100) f100 on fu.drupal_user_id = f100.drupal_user_id;

select max_non_null_bin, count(1)
from fve_min_bins

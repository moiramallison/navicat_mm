...
select foo.subscription_id, 
       ...
       case when dss2.paid_through_date::date >= '20160315' and 
                 dss2.paid_through_date::date <= '20160414' and
                 dss2.status = 'Cancelled'
            then 1
						when dss2.paid_through_date::date >= '20160301' and 
                 dss2.paid_through_date::date <= '20160401' and
                 dss2.status = 'Suspended'
            then 1
            else 0
       end churner
from 
   (select dss.subscription_id,
          ...
          '20160414'::date - sd.start_date::date subscription_age
   ...
   where dss.day_timestamp = '20160414'
     and dss.paid_through_date >=  '20160414') foo
   left join common.daily_status_snapshot dss2 
      on foo.subscription_id = dss2.subscription_id and 
         dss2.day_timestamp = '20160414'
